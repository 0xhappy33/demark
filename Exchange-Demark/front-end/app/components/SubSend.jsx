...
// const DTUInstance = new web3.eth.Contract(
//   DTUAbi, "0xF92bbac6a4e9bD4a9B4b53015ED6A0bc1ca6b1E6"
// );
const DTUInstance = web3.eth.contract(DTUAbi).at("0xF92bbac6a4e9bD4a9B4b53015ED6A0bc1ca6b1E6");

const callTransfer = async (instance,currentAcc, to, amount) =>{
  return new Promise((resolve, reject) => {
    instance.transfer.sendTransaction(to, amount, {from:currentAcc},(err,result)=>{
      if(err) resolve(err);
      resolve(result);
    })
  });
...
class SubSend extends React.Component {
  constructor(props){
    super(props);
  ...
  async onSubmitTransfer(e) {
    ...
    try {
      ...
      // console.log("dtu ", web3.eth.defaultAccount);

      await callTransfer(DTUInstance,accounts,this.state.recipient,this.state.amount);
      // await DTUInstance.transfer.sendTransaction(this.state.recipient, this.state.amount, (err,result)=>{
  ...
  render() {
    return (
      <div>
        <form className="form-horizontal" role="form" onSubmit={this.onSubmitTransfer} >
        <Input type="text" ref="address"
          placeholder="0x"
          label={<FormattedMessage id='form.recipient' />} 
        ...
        <Input type="number" ref="amount"
          label={<FormattedMessage id='form.amount' />} 
          labelClassName="sr-only"
